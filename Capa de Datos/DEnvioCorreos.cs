...
namespace Capa_de_Datos
{
    public class DEnvioCorreos
    {
            ...
            try
            {
                using (var context = new SnowDentEntities7())
                {
                    ...
                    string cuerpo = $"Hola {paciente.Nombre},\n\n" +
                                    $"Te recordamos que tienes una cita programada para el día {cita.Fecha:dd/MM/yyyy} a las {cita.Hora:hh\\:mm} horas.\n\n" +
        ...
        // Envía un correo de confirmación de cita al paciente al momento de registrar la cita
        public string EnviarConfirmacionCita(string pacienteDNI, DateTime fecha, TimeSpan hora)
        {
            try
            {
                using (var context = new SnowDentEntities7())
                {
                    ...
                    string cuerpo = $"Hola {paciente.Nombre},\n\n" +
                                    $"Tu cita ha sido registrada para el día {fecha:dd/MM/yyyy} a las {hora:hh\\:mm} horas.\n\n" +
                                    "Por favor, no olvides asistir.\n\n" +
                                    "Saludos,\nClínica Dental SnowDent";
